function signal = d2d_clean_asr(signal,cutoff,windowlen,stepsize,maxdims,ref_maxbadchannels,ref_tolerances,ref_wndlen,usegpu,useriemannian,maxmem)
...
%
%   MaxMem : Amount of memory to use. See d2d_asr_process for more information.
%
...
% Adapted by Richard Somervail in 2023, for the Dusk2Dawn plugin for EEGLAB
...
if exist('hlp_diskcache','file')
    if useriemannian
        state = hlp_diskcache('filterdesign',@d2d_asr_calibrate_r,ref_section.data,ref_section.srate,cutoff); % ? RS: not editing these lines because not sure how paramters are passed to this (external?) version of the function
    else
        state = hlp_diskcache('filterdesign',@d2d_asr_calibrate,ref_section.data,ref_section.srate,cutoff); % ? RS: not editing these lines because not sure how paramters are passed to this (external?) version of the function
    end
else
    if useriemannian
        state = d2d_asr_calibrate_r(ref_section.data,ref_section.srate,cutoff, [], [], [], windowlen, [], [], [], maxmem); % RS edit to pass down window length
%         state = d2d_asr_calibrate_r(ref_section.data,ref_section.srate,cutoff, [], [], [], [], [], [], [], maxmem); % original code
    else
        state = d2d_asr_calibrate(ref_section.data,ref_section.srate,cutoff, [], [], [], windowlen, [], [], [], maxmem); % RS edit to pass down window length
%         state = d2d_asr_calibrate(ref_section.data,ref_section.srate,cutoff, [], [], [], [], [], [], [], maxmem); % original code
    end
end
clear ref_section;
...
if useriemannian
    [signal.data,state] = d2d_asr_process_r(sig,signal.srate,state,windowlen,windowlen/2,stepsize,maxdims,maxmem,usegpu);
else
    [signal.data,state] = d2d_asr_process(sig,signal.srate,state,windowlen,windowlen/2,stepsize,maxdims,maxmem,usegpu);
end
