...
if exist('hlp_diskcache','file')
    if useriemannian
        state = hlp_diskcache('filterdesign',@d2d_asr_calibrate_r,ref_section.data,ref_section.srate,cutoff);
    else
        state = hlp_diskcache('filterdesign',@d2d_asr_calibrate,ref_section.data,ref_section.srate,cutoff);
    end
else
    if useriemannian
        state = d2d_asr_calibrate_r(ref_section.data,ref_section.srate,cutoff, [], [], [], [], [], [], [], maxmem);
    else
        state = d2d_asr_calibrate(ref_section.data,ref_section.srate,cutoff, [], [], [], [], [], [], [], maxmem);
    end
end
clear ref_section;
